...
*
* Every resource is backed by a tool. Reading gmcp://post/12 is the same read as
...
class GMCP_Resources {
  ...
  */
  public static function read( string $uri, callable $permitted ): ?array {
    ...
    if ( preg_match( '#^gmcp://post/(\d+)$#', $uri, $m ) ) {
      if ( !$permitted( 'wp_get_posts' ) ) {
        return null;
      }
  ...
  private static function render_fixed( string $uri ): ?string {
    if ( $uri === 'gmcp://site/briefing' || $uri === 'gmcp://site/health' ) {
      ...
      $tools = apply_filters( 'gmcp_tools', [] );
      $names = array_column( $tools, 'name' );
      $needed = $uri === 'gmcp://site/briefing' ? 'wp_site_briefing' : 'wp_get_site_health';
    ...
    switch ( $uri ) {
      case 'gmcp://site/briefing':
      case 'gmcp://site/health':
        // Rendered through the tool itself rather than reimplemented, so the two can
        // never drift and a fix in one is a fix in both.
        return self::via_tool( $uri === 'gmcp://site/briefing' ? 'wp_site_briefing' : 'wp_get_site_health' );

      case 'gmcp://comments/pending':
  ...
  private static function via_tool( string $tool, array $args = [] ): ?string {
    $result = apply_filters( 'gmcp_callback', null, $tool, $args, 0 );
    if ( !is_array( $result ) ) {
      return null;
    }
