...
- Anything that emails a customer says so. Moving an order to processing, completed, cancelled or refunded triggers a WooCommerce notification, and the tool tells you one has probably gone out. Setting the status to refunded marks the order and moves no money.
...
**Preview.** The six tools whose effect is not visible from the call accept `preview`, which describes what would happen and changes nothing: `wp_delete_post`, `wp_update_post`, `wp_alter_post`, `wp_delete_term`, `wp_delete_media` and `wp_delete_comment`. It matters most for `wp_alter_post`, where a search and replace reports success whether it matched everything or nothing. The preview compiles the pattern through the same function the write uses, shows every match in context with its replacement, and says plainly when a pattern matches nothing.
